...
def fetch_stock_data(tickers_tuple: tuple[str, ...]) -> pl.DataFrame:
    ...
    log_returns_df = adjclose_df.select(
        [pl.col("date")] + [np.log(pl.col(c) / pl.col(c).shift(1)).alias(c) for c in tickers]
    ).drop_nulls()
...
class StockPortfolioAsset(BaseAsset):
    ...
    def _calculate_portfolio_metrics(self, mix: dict):
        ...
        stock_return, stock_volatility = calculate_portfolio_params(stock_log_returns, mix)
    ...
    def modify(self, updates: dict):
        ...
        if "portfolio_mix" in updates:
            ...
            logger.info(f"Updated StockPortfolioAsset expected return: {self.asset_params.expected_return} -> {new_metrics['expected_return']}")
            logger.info(f"Updated StockPortfolioAsset volatility: {self.asset_params.volatility} -> {new_metrics['volatility']}")
